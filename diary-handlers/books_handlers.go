...
func GetUsersBooks(db *gorm.DB) func(c echo.Context) error {
  return func(c echo.Context) error {
    ...
    if user, err := GetUser(c, db); err == nil {
      db.Table("books").Select(
        "books.id, books.title, books.author, books.image_url, user_book.in_books, user_book.status, user_book.in_books").
    ...
    if loggedUser, logErr := GetUser(c, db); logErr == nil {
      if id, err := strconv.Atoi(c.Param("id")); err == nil {
        if ( db.Table("books").Select(
          "books.id, books.title, books.author, books.image_url, user_book.status, user_book.in_books", ).
